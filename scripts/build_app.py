...
@media (max-width:1000px){#main{grid-template-columns:1fr;height:auto}#map{height:55vh;position:relative;border-left:0;border-top:1px solid var(--line)}#tablewrap{max-height:none}}
</style>
</head>
<body>
<header>
...
<div id="filters">
  <div class="fgroup" id="f-banco"><label>Banco</label></div>
  <div class="fgroup" id="f-dia"><label>Día</label></div>
  <div class="fgroup" id="f-tarj"><label>Tarjeta</label></div>
  <div class="fgroup"><label>Comuna</label><select id="sel-com"><option value="">(agregar comuna…)</option></select><span id="f-com" class="fgroup"></span></div>
  <div class="fgroup"><label>Buscar</label><input type="text" id="q" placeholder="comercio, lugar, condición…"></div>
...
function selectRow(id, fromTable){
  ...
  if(ms.length){ const b=L.latLngBounds(ms.map(m=>m.getLatLng())); if(ms.length===1){ map.setView(ms[0].getLatLng(), Math.max(map.getZoom(),16)); cluster.zoomToShowLayer(ms[0],()=>ms[0].openPopup()); } else map.fitBounds(b.pad(0.3)); }
}
